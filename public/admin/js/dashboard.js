...
async function editBusiness(event) {
    ...
    try {
        ...
        // Scroll ke form edit bisnis setelah scroll ke atas
        const editBusinessSection = document.getElementById('editBusinessSection');
        if (editBusinessSection) {
            editBusinessSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }

        // Fokus ke input nama bisnis jika elemen ada
        const updateBusinessName = document.getElementById('updateBusinessName');
        if (updateBusinessName) {
            updateBusinessName.focus();
        } else {
            console.error('Elemen input nama bisnis tidak ditemukan!');
        }
...
async function updateBusiness(businessId) {
    const updatedBusiness = {
        ...
        category_name: document.getElementById('editCategorySelect').options[document.getElementById('editCategorySelect').selectedIndex].text,  // Ambil nama kategori, bukan ID
    ...
    if (result.isConfirmed) {
        try {
            ...
            if (deleteResponse.ok) {
                ...
                displayAlert('Error!', 'Terjadi kesalahan saat menghapus kategori.', 'error');
            }
        } catch (error) {
