...
app.use(
  session({
    secret: "semantic-web-batam", // Ganti dengan secret key Anda
    resave: false,
...
const db = mysql.createConnection({
  host: "bqzcxkupakcc5rrtnfqn-mysql.services.clever-cloud.com",
  user: "updunxkckdzcru5n",
  password: "kpCIc6CG7dauwzzJPDMX",
  database: "bqzcxkupakcc5rrtnfqn", // Ganti dengan nama database Anda
});
...
app.get("/admin/isLoggedIn", (req, res) => {
  res
    .status(req.session.isAdmin ? 200 : 401)
    .json({ isLoggedIn: req.session.isAdmin });
});
...
app.post("/categories", (req, res) => {
  ...
  db.query(query, [nama_kategori], (err) => {
    if (err) {
      ...
      return res
        .status(500)
        .send("Terjadi kesalahan saat menambahkan kategori");
    }
...
app.put("/categories/:id", (req, res) => {
  ...
  db.query(query, [nama_kategori.trim(), id], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Terjadi kesalahan saat memperbarui kategori." });
    }
...
app.get("/businesses/category/:categoryName", (req, res) => {
  ...
  const query = `
    ...
    WHERE jenis_usaha = ?
    AND (nama_usaha LIKE ? OR jenis_usaha LIKE ?)
    LIMIT ? OFFSET ?
  `;
  db.query(
    query,
    [categoryName, `%${searchQuery}%`, `%${searchQuery}%`, limit, offset],
    (err, results) => {
      if (err) {
        return res
          .status(500)
          .send("Terjadi kesalahan saat mengambil data bisnis");
      }

      // Query untuk menghitung total bisnis
      const countQuery = `
      SELECT COUNT(*) AS total FROM businesses
      WHERE jenis_usaha = ? AND (nama_usaha LIKE ? OR jenis_usaha LIKE ?)
    `;
      db.query(
        countQuery,
        [categoryName, `%${searchQuery}%`, `%${searchQuery}%`],
        (err, countResult) => {
          if (err) {
            return res
              .status(500)
              .send("Terjadi kesalahan saat menghitung total bisnis");
          }
...
app.post("/businesses", upload.single("businessPhoto"), (req, res) => {
  ...
  const { nama_usaha, jenis_usaha, alamat, no_hp, website, category_name } =
    req.body;
  const businessPhoto = req.file.filename;

  const checkCategoryQuery =
    "SELECT * FROM categories WHERE LOWER(nama_kategori) = LOWER(?)";
  db.query(checkCategoryQuery, [category_name], (err, results) => {
    ...
    const insertQuery =
      "INSERT INTO businesses (nama_usaha, jenis_usaha, alamat, no_hp, website, category_name, foto) VALUES (?, ?, ?, ?, ?, ?, ?)";
    db.query(
      insertQuery,
      [
        nama_usaha,
      ...
      (err) => {
        if (err) {
          return res
            .status(500)
            .send("Terjadi kesalahan saat menambahkan bisnis");
        }
...
app.put("/businesses/:id", upload.single("foto"), (req, res) => {
  ...
  db.query(query, [businessId], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Terjadi kesalahan saat mengambil data bisnis." });
    }
    ...
    db.query(
      updateQuery,
      [nama_usaha, category_name, alamat, no_hp, website, foto, businessId],
      (err, result) => {
        if (err) {
          console.error("Kesalahan saat memperbarui bisnis:", err);
          return res
            .status(500)
            .json({ error: "Terjadi kesalahan saat memperbarui bisnis." });
        }
  ...
  db.query(query, [businessId], (err, result) => {
    ...
    db.query(deleteQuery, [businessId], (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Terjadi kesalahan saat menghapus bisnis." });
      }
